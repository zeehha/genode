...
Vmm::Cpu & Vm::boot_cpu()
{
	if (!_cpus[0].constructed())
		_cpus[0].construct(*this, _vm, _bus, _gic, _env, _heap, _env.ep(), _snapshotter);
	return *_cpus[0];
}
  ...
  _snapshotter(env, _vm, _vm_ram)
{
	...
	for (unsigned i = 1; i < MAX_CPUS; i++) {
		...
		_cpus[i].construct(*this, _vm, _bus, _gic, _env, _heap, *_eps[i], _snapshotter);
	}
